...
var validateField = function(field) {
    ...
    if (hasVal && validations.indexOf('past-date') > -1 && fieldDate >= d) {
        messages.push(fieldName + " accepts only past date."); 
        isValid =  false;
    }
    ...
    var errorContainerTemplate = $('<ul class="list-unstyled text-error" data-error-id="' + field.prop("name") + '" />');
    var inputContainer = field.closest("div");

    if (!isValid) {
        field.addClass('input-error')
        if (!errorContainer.length) {
            inputContainer.append(errorContainerTemplate.html(listErrors(messages)).css({'display': 'inline-block'}));
        } 
        errorContainer.html(listErrors(messages)).css({'display': 'inline-block'});
        return isValid;
    } 
